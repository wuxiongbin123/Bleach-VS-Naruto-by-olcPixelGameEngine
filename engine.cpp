...
Unit::Unit(bool s, unitType u): S(stand), lives(100)
                                ...
                                attackNum(0), attackTime(clock() - 500),
...
bool Example::OnUserUpdate(float fElapsedTime) {
    ...
    if (GetKey(olc::Key::RIGHT).bHeld){
        switch (unitB.S) {
            ...
            case jump:{
                unitB.face = true;
                if (unitB.position.x + unitB.speed.x * fElapsedTime <= ScreenWidth() - blockSize.x)
...
//recover进行状态转移。
void Example::recover() {
    //当松开方向键时且没有跳跃时，将状态设定为静止。
    if ((GetKey(olc::Key::RIGHT).bReleased || GetKey(olc::Key::LEFT).bReleased)
        && unitB.S != jump && unitB.S != attack && unitB.S != hit) unitB.S = stand;
    switch(unitB.S){
        case attack:{
            if( (clock() - unitB.attackTime) > 500) unitB.S = stand;
        }break;
        case hit:{
            if ((clock() - unitB.hitTime) > 500) unitB.S = stand;
        }break;
    }

    //当松开方向键时且没有跳跃时，将状态设定为静止。
    if ((GetKey(olc::Key::D).bReleased || GetKey(olc::Key::A).bReleased)
        && unitA.S != jump && unitA.S != attack && unitA.S != hit) unitA.S = stand;
    switch(unitA.S){
        case attack:{
            if( (clock() - unitA.attackTime) > 500) unitA.S = stand;
        }break;
        case hit:{
            if ((clock() - unitA.hitTime) > 500) unitA.S = stand;
        }break;
    }
}
...
void Example::attackAction(Unit &unit, float fElapsedTime) {
    ...
    if (unit.S != jump && (clock() - unit.attackTime) > 500) {
        unit.S = attack;
        unit.attackNum = (unit.attackNum + 1) % 3;
        //更新攻击时间
